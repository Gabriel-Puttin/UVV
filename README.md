🎓 Repositório de Códigos da Faculdade

Este repositório reúne todos os códigos, projetos e atividades desenvolvidos durante minha graduação.
O objetivo é organizar e documentar minha evolução acadêmica, além de servir como material de consulta futura.

📚 Estrutura do Repositório

Cada pasta corresponde a uma disciplina cursada ao longo do curso.
Dentro de cada pasta, estão organizadas as atividades, trabalhos, projetos e exercícios relacionados.

📦 faculdade-codigos

 ┣ 📂 nome-da-materia-1
 
 ┃ ┣ 📜 atividade-1
 
 ┃ ┣ 📜 atividade-2

 ┃ ┗ 📜 projeto-final
 
 ┣ 📂 nome-da-materia-2
 
 ┃ ┣ 📜 lista-exercicios
 
 ┃ ┗ 📜 trabalho-pratico
 
 ┗ 📜 README.md

🧠 Disciplinas

***
1° PERÍODO

📌 Construção de Software para Web

📌 Design e Desenvolvimento de Banco de Dados I

📌 Experiência e Interface com o Usuário

📌 Fundamentos de Tecnologia da Computação

📌 Lógica para Computação

📌 Textos Científicos: Aspectos Metodológicos e Linguísticos

***
2° PERÍODO

📌 Álgebra para Computação

📌 Engenharia de Software I

📌 Engenharia Transformacional

📌 Laboratório de Programação

📌 Marketing Digital

***
3° PERÍODO

📌 Design e Desenvolvimento de Banco de Dados II

📌 Engenharia de Software II

📌 Estatística Básica

📌 Inovação e Design Thinking

📌 Programação Orientada a Objetos I

***
4° PERÍODO

📌 Arquitetura e Organização de Computadores

📌 Aspectos Legais da Tecnologia da Informação

📌 Estrutura de Dados

📌 Programação Orientada a Objetos II

📌 Qualidade de Software

📌 Sistemas Operacionais

***
5° PERÍODO

📌 Desenvolvimento Pessoal e Profissional

📌 Nome da Matéria 2

📌 Estatística para Ciência de Dados

📌 Networking Fundamentals

📌 Processamento de Grandes Volumes de Dados

📌 Programação Avançada para WEB

📌 Teste de Software

***
6° PERÍODO

📌 Data Science

📌 Desenvolvimento Mobile

📌 Gestão de Processos

📌 Gestão de Projetos

📌 Inteligência Artificial

📌 Inteligência de Mercado

***
7° PERÍODO

📌 Arquitetura Corporativa e Tecnologia da Informação

📌 Empreendedorismo e Negócios

📌 Governança em Tecnologia da Informação

📌 Laboratório de Engenharia de Software

📌 Projeto Final de Curso em Sistemas de Informação I

📌 Sistemas de Apoio à Tomada de Decisão

***
8° PERÍODO

📌 Atividades Complementares em Sistemas de Informação

📌 Cloud Computing

📌 Cybersecurity

📌 Estágio Supervisionado em Sistemas de Informação

📌 Optativa (1)

📌 Projeto Final de Curso em Sistemas de Informação II

***

🛠️ Tecnologias Utilizadas

Ao longo do curso, foram utilizadas diferentes linguagens e ferramentas, como:

💻 Linguagens:

JavaScript

Python

SQL

Java

C# (.NET)

🗄️ Banco de Dados:

MySQL

Oracle

Redis

🔧 Ferramentas:

Alice

GreenFoot

Workbench

Figma

Astah

VsCode

Apache NetBeans

***
🚀 Objetivo

Este repositório tem como finalidade:

Organizar meus projetos acadêmicos

Demonstrar minha evolução técnica

Servir como portfólio acadêmico

Facilitar revisões futuras

📌 Observações

Alguns códigos podem conter versões iniciais de aprendizado e não necessariamente representam as melhores práticas atuais.

